...
def chat():
    ...
    # ====================================
    # PENYARINGAN DATA STATIS (FUZZYWUZZY)
    # ====================================
    # Mencari kemiripan pesan dengan daftar pertanyaan FAQ tanpa memanggil API
    best_match = process.extractOne(message, faq_questions)
    
    # Jika tingkat kemiripan di atas 75%, langsung ambil jawaban dari JSON
    if best_match and best_match[1] >= 75:
        ...
        for item in faq_data:
            if item["question"] == matched_question:
                reply_statis = item["answer"]
                break
                
        save_log(user_id, message, reply_statis)
        
        return jsonify({
            "reply": reply_statis,
            "similarity_score": float(best_match[1]) / 100.0,
            "retrieved_faq": [{"question": matched_question, "answer": reply_statis, "score": float(best_match[1]) / 100.0}]
        })
...
# Tambahkan ini agar semua request masuk ke fungsi whatsapp
